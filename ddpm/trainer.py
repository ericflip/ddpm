...
def generate_samples(model: UNet, diffusion: GaussianDiffusion, grid_size=8):
    pipeline = DDPMPipeline(model, diffusion)
...
class Trainer:
    ...
    def generate_and_save_samples(self, epoch: int = None):
        ...
        samples = generate_samples(
            self.model, self.diffusion, grid_size=int(self.num_samples**0.5)
        )
